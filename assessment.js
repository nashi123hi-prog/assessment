'use strict'; // 厳格モード 変数宣言していないものをエラー出力するもの
...
assessmentButton.addEventListener( //イベント検知設定の追加
  'click', //クリックイベント
  () => { //無名関数でアロー関数です。functionと同じ意味を持ちます。
    const userName = userNameInput.value // 入力欄(input)の値を取得する
    if (userName.length === 0) {//入力が空だったら
      return; //関数の処理を終了
    }

    //診断表示エリアの作成
    resultDivision.innerText = '' // divタグを空文字で上書きすることで空にする
    const header = document.createElement('h3') //h3タグの作成
    header.innerText = '診断結果'//タグの内側のテキストを固定
    ...
    anchor.innerText = '#あなたのいいいところをポストする。'//ボタンの文章を追加

    tweetDivision.appendChild(anchor) // divの子要素として追加


    const script = document.createElement('script')//scriptタグを作成
    script.setAttribute('src','https://platform.x.com/widgets.js')//src属性を追加
